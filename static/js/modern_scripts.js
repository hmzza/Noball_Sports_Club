...
class ModernNoBall {
    ...
    setupContactMethods() {
        ...
        const phoneAnchor = document.querySelector('.contact-method .method-value a[href^="tel:"]');
        const whatsappAnchor = document.querySelector('.contact-method .method-value a[href*="wa.me"]');
        const cleanNumber = (value = '') => value.replace(/[^\d+]/g, '');
        const extractWhatsAppNumber = (href = '') => {
            try {
                const url = new URL(href, window.location.origin);
                return cleanNumber(url.pathname.replace(/\//g, '')) || cleanNumber(url.search.replace(/[^\d]/g, ''));
            } catch (e) {
                return '';
            }
        };
        const fallbackPhone = '+923293180180';
        const dialNumber = cleanNumber((phoneAnchor?.getAttribute('href') || '').replace(/^tel:/, '')) || fallbackPhone;
        const waNumber = extractWhatsAppNumber(whatsappAnchor?.getAttribute('href') || '') || '923293180180';
        ...
        contactMethods.forEach(method => {
            method.addEventListener('click', () => {
                ...
                if (iconElement.classList.contains('fa-phone')) {
                    window.location.href = `tel:${dialNumber.startsWith('+') ? dialNumber : '+' + dialNumber}`;
